...
export const getMe = asyncHandler(async (req, res, next) => {
    ...
    if (!me) return next(new Error("المستخدم غير موجود", { cause: 404 }));
...
export const updateProfile = asyncHandler(async (req, res, next) => {
    const currentUserId = req.user.id;
    const { fullname, phone, city, place, district } = req.body;

    const updated = await findByIdAndUpdate({
        model: userModel,
        id: currentUserId,
        data: { fullname, phone, city, place, district },
...
export const createUser = asyncHandler(async (req, res, next) => {
    ...
    if (exists) return next(new Error("المستخدم موجود بالفعل"));
...
export const updateUser = asyncHandler(async (req, res, next) => {
    const { id } = req.params;
    const { fullname, email, phone, role, password, city, place, district } = req.body;

    const data = { fullname, email, phone, role, city, place, district };
    ...
    if (!updated) return next(new Error("المستخدم غير موجود", { cause: 404 }));
...
export const deleteUser = asyncHandler(async (req, res, next) => {
    ...
    if (!deleted) return next(new Error("المستخدم غير موجود", { cause: 404 }));
    return successResponse({ res, data: { message: "تم حذف المستخدم" } });
});